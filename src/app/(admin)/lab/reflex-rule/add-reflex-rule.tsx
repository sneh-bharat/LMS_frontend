...
interface AddReflexRuleProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateReflexRuleInput) => void;
  editData?: ReflexRule | null;
}

export default function AddReflexRule({
  isOpen,
  onClose,
  onSubmit,
  editData,
  ...
  const [reflexTests, setReflexTests] = useState<Test[]>([]);
  ...
  const validateForm = (): boolean => {
    ...
    // POSITIVE, NEGATIVE, ABNORMAL, and ALWAYS don't require threshold values
  ...
  const handleTestSelect = (test: Test) => {
    setFormData((prev) => ({
      ...
      parameterId: null, // Reset parameter when test changes
      parameterName: '',
    }));
    // Fetch parameters for the selected test
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const payload: CreateReflexRuleInput = {
      ...
      // Pass payload to parent component - parent handles API calls
      onSubmit(payload);
      setLoading(false);
    } catch (error) {     console.error('Failed to prepare reflex rule:', error);
      setLoading(false);
    }
  };
  ...
  const footer = (
    <div className="flex gap-3 w-full">
      ...
      <Button
        ...
        disabled={loading}
      >
        {loading ? 'Saving...' : editData ? 'Update Rule' : 'Create Rule'}
      </Button>
    </div>
  );
  ...
  return (
    <RightDrawer
      ...
      description={editData ? 'Update reflex rule configuration' : 'Add a new automated reflex testing rule'}
      footer={footer}
      maxWidth="lg"
    >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setFormData((prev) => ({
                    ...prev,
                    parameterId: (param?.id as number) || null,
