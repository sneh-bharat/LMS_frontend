...
const EMPTY_TESTS: ReferrerTestCommission[] = [];
const PAGE_SIZE = 10;
...
export default function GetTestCommission({
  ...
  const [testToUpdate, setTestToUpdate] = useState<ReferrerTestCommission | null>(null);

  const activeReferrerId = referrerId != null && referrerId > 0 ? referrerId : null;
  const activeReferrerName = referrerName?.trim() || 'Referrer';

  const testsQuery = useReferrerTestCommissions(activeReferrerId, {
    enabled: isOpen && activeReferrerId != null,
  });
      ...
      >
        {!activeReferrerId ? (
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 font-medium">
            Referrer ID is required to load test commissions.
          </div>
        ) : loading ? (
            ...
            {tests.length === 0 ? (
              <div className="p-10 rounded-xl border border-dashed border-slate-200 text-center">
                ...
                <p className="text-sm text-slate-500 font-medium">
                  No test commissions found for this referrer.
                </p>
              </div>
