...
import { SAMPLE_INVOICES } from '../invoice-list/constants';
import { Invoice } from '../invoice-list/types';
...
export default function InvoiceDetailsPage() {
  ...
  // Find invoice data
  const invoice = SAMPLE_INVOICES.find(inv => inv.invoiceBarcode === invoiceId);
  ...
  const [form, setForm] = useState<FormState>({
    ...
    age: invoice?.age?.toString() || '',
    gender: invoice?.gender || 'Male',
  ...
  const [investigations, setInvestigations] = useState<Investigation[]>(
    invoice?.tests.map((test, idx) => ({
      id: idx + 1,
      name: test,
      mrp: 500, // Default MRP, you can adjust based on actual data
      category: 'General'
    })) || []
  );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm font-semibold mt-1 flex items-center gap-2">
              Viewing Invoice <span className="w-1 h-1 rounded-full bg-slate-300"></span>
              <span className="text-emerald-600 font-bold">{invoice?.invoiceBarcode || 'N/A'}</span>
            </p>
          </div>
        </div>
