...
import {
  ...
  type MemberCardBalanceData,
  type MemberCard,
} from '@/app/Apis/membership/membership';
import { useMemberCardBalanceById, useMemberCardById } from '@/app/Apis/membership/useMembership';
...
function DetailTile(props: DetailTileProps) {
  const { label, value, icon: Icon, mono, highlight } = props;
  const valueClass =
    highlight === 'emerald'
      ? 'text-emerald-700'
      : highlight === 'amber'
...
function pickBalanceAmount(...values: (number | undefined)[]): number | null {
...
type MemberCardDetailsBodyProps = {
  card: MemberCard;
  balance?: MemberCardBalanceData;
};

function MemberCardDetailsBody(props: MemberCardDetailsBodyProps) {
...
export default function CardDetails(props: MemberDetailsViewProps) {
  const { isOpen, onClose, cardId, onEdit } = props;
  const [copied, setCopied] = useState(false);
  const { data, isLoading, isError, error, refetch } = useMemberCardById(cardId, {
    enabled: isOpen && cardId != null,
  });
  const { data: balanceRes } = useMemberCardBalanceById(cardId, {
    enabled: isOpen && cardId != null,
  });
    ...
    >
      {isLoading ? (
        ...
        <>
          ...
          <MemberCardDetailsBody card={card} balance={balance} />
        </>
      ) : (
