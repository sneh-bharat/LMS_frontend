...
function normalizeRangeTest(raw: unknown): ReferrerCommissionRangeTestItem | null {
...
function normalizePaySummary(data: ReferrerCommissionPaySummary): ReferrerCommissionPaySummary {
  const raw = data as ReferrerCommissionPaySummary & {
    tests?: unknown[];
    doctorId?: number;
  };
  const referrerId = raw.referrerId ?? raw.doctorId ?? 0;
  const rawTests = raw.tests;
  const tests = Array.isArray(rawTests)
    ? rawTests
        .map((item) => normalizeRangeTest(item))
        .filter((item): item is ReferrerCommissionRangeTestItem => item != null)
    : [];

  return { ...data, referrerId, tests };
}

export interface CalculateRffProps {
  isOpen: boolean;
  onClose: () => void;
...
function CommissionTestTable({ tests }: { tests: ReferrerCommissionRangeTestItem[] }) {
  if (tests.length === 0) {
    return (
...
function CommissionRangeResults({
  ...
  summary: ReferrerCommissionPaySummary;
  referrerName: string;
  startDate: string;
  endDate: string;
}) {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 pt-2">
      <div className="flex items-start justify-between bg-slate-50 p-5 rounded-2xl border border-slate-100">
        <div className="flex items-center gap-4 min-w-0">
          <div className="w-12 h-12 bg-orange-100 rounded-2xl flex items-center justify-center text-[#ff671f] shrink-0">
            <Building2 size={22} aria-hidden />
          </div>
          <div className="min-w-0">
            <h3 className="text-lg font-bold text-slate-900 tracking-tight truncate">{referrerName}</h3>
            <div className="flex flex-wrap items-center gap-2 mt-1.5">
              <Badge variant="secondary" className="text-[10px] font-bold border-slate-200 font-mono">
                Referrer #{summary.referrerId}
              </Badge>
...
export default function CalculateRff({
  isOpen,
  onClose,
  referrerId,
  referrerName,
  ...
  const [summary, setSummary] = useState<ReferrerCommissionPaySummary | null>(null);

  const calculateMutation = useCalculateReferrerCommissionByRange();
  ...
  const handleCalculate = () => {
    if (!referrerId || referrerId < 1) {
      toast.error('Select a referrer first.');
      return;
    }
    if (!validate()) return;
  ...
  return (
    <RightDrawer
      ...
      description="Calculate referrer commission for a specific period"
      maxWidth="xl"
      footer={
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-5">
          <div className="space-y-2">
            ...
            <button
              ...
              disabled
              aria-label="Selected referrer"
            >
              <span className="truncate">
                {displayReferrerName}
                {referrerId != null && referrerId > 0 ? (
                  <span className="text-slate-500 font-semibold ml-2">(ID: {referrerId})</span>
                ) : null}
              </span>
            ...
            {!referrerId || referrerId < 1 ? (
              <p className="text-xs text-amber-700 flex items-center gap-1 font-medium">
                <AlertCircle size={12} aria-hidden /> Open from a referrer row to select a referrer.
              </p>
            ) : null}
          </div>
          ...
          <Button
            ...
            disabled={pending || !referrerId || referrerId < 1 || dateRangeInvalid}
