...
export default function BranchManagerListPage() {
  ...
  const {
    ...
    { enabled: !isUsernameApiSearch && filterMode === "all" },
  );
  ...
  const {
    ...
    page: pageNo,
    size: PAGE_SIZE,
    enabled: !isUsernameApiSearch && filterMode === "active",
  });
  ...
  const {
    ...
    page: pageNo,
    size: PAGE_SIZE,
    enabled: !isUsernameApiSearch && filterMode === "verified",
  });
  ...
  const totalPages = isUsernameApiSearch
    ? 1
  ...
  const canNext =
    !isUsernameApiSearch && activePageData ? !activePageData.last : false;
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-1 px-4 py-2.5 bg-slate-50/60">
            ...
            {FILTER_TABS.map(({ mode, label, icon }) => {
              const isActive = filterMode === mode && !isUsernameApiSearch;
              return (
                <button
                  key={mode}
                  type="button"
